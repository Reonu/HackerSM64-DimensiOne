...
void tuxies_mother_act_received_baby(void) {
    ...
    f32 diffY = ABS(gMarioState->pos[1] - 250.0f - o->oPosY);
...
u8 check_penguin_on_lava(void) {
    if (o->oFloorType == SURFACE_BURNING && o->oFloorHeight >= o->oPosY) {
        return TRUE;
    }
...
void bhv_small_penguin_loop(void) {
    ...
    if (check_penguin_on_lava()) {
        if (is_challenge_active()) {
            ...
            if (
                (get_challenge_required_flags() & CHALLENGE_FLAG_KILL_PENGUIN) &&
                !(get_challenge_obtained_flags() & CHALLENGE_FLAG_KILL_PENGUIN)
            ) {
                play_puzzle_jingle();
            }
